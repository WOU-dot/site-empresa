...
function getScreen2GlobePos() {
  if (window.innerWidth <= 600) return { x: W * 0.50, y: H * 0.50, scale: 0.55 };
  if (window.innerWidth <= 900) return { x: W * 0.50, y: H * 0.50, scale: 0.58 };
  return { x: W * 0.50, y: H * 0.50, scale: 0.65 };
}
...
═══════════════════════════════════════════════════════ */
/* data-footer-nav="N" → navega para screen N (horizontal) */
document.querySelectorAll('[data-footer-nav]').forEach(a => {
  a.addEventListener('click', e => {
    e.preventDefault();
    updateNav(parseInt(a.getAttribute('data-footer-nav')));
    window.scrollTo({ top: 0, behavior: 'smooth' });
  });
});

/* data-footer-scroll="N" → vai para o scroll-section N dentro de screenInicio
   0=topo(globo), 1=proposta, 2=método, 3=depoimentos, 4=faq */
const footerScrollTargets = [0, 1.7, 2.7, 3.8, 5.3];
document.querySelectorAll('[data-footer-scroll]').forEach(a => {
  a.addEventListener('click', e => {
    e.preventDefault();
    updateNav(0);
    const idx = parseInt(a.getAttribute('data-footer-scroll'));
    const target = (footerScrollTargets[idx] || 0) * window.innerHeight;
...
/* Footer contact form */
(function () {
  document.querySelectorAll('#footerContactForm select').forEach(sel => {
    sel.addEventListener('change', () => sel.classList.toggle('filled', sel.value !== ''));
  });
  const form    = document.getElementById('footerContactForm');
  const btn     = document.getElementById('footerBtnSubmit');
  const success = document.getElementById('footerFormSuccess');
  if (!form) return;
...
/* ═══════════════════════════════════════════════════════
   PORTFOLIO — PNG SEQUENCE HOVER PREVIEW
═══════════════════════════════════════════════════════ */
(function () {
  document.querySelectorAll('.pcard').forEach(card => {
    const video  = card.querySelector('.pcard-video');
    const thumb  = card.querySelector('.pcard-thumb');
    const canvas = card.querySelector('.pcard-canvas');
    const ctx    = canvas.getContext('2d');

    let active    = false;   /* mouse está sobre o card */
    let seeking   = false;   /* seek em andamento */
    let frameTime = 0;
    let duration  = 0;
    let step      = 1;

    /* ── Desenha frame com object-fit: cover, sem esticar ── */
    function drawCover() {
      ...
      if (videoAR > canvasAR) {          /* vídeo mais largo → corta laterais */
        sh = vh; sw = vh * canvasAR;
        sx = (vw - sw) / 2; sy = 0;
      } else {                           /* vídeo mais alto → corta topo/baixo */
    ...
    /* ── Sincroniza resolução do canvas com tamanho CSS ── */
    function syncCanvas() {
    ...
    /* ── Dispara seek; aguarda 'seeked' antes de pedir o próximo ── */
    function seekNext() {
      if (!active || duration <= 0) { seeking = false; return; }
      frameTime += step;
      if (frameTime >= duration) frameTime = 0;
      seeking = true;
      if (video.fastSeek) video.fastSeek(frameTime);
    ...
    video.addEventListener('seeked', () => {
      ...
      drawCover();
      seeking = false;
      if (active) setTimeout(seekNext, 150); /* ~6-7 fps suave, sem travar */
    });

    /* ── Define aspect-ratio real do container a partir do vídeo ── */
    video.addEventListener('loadedmetadata', () => {
      duration = video.duration;
      step = Math.max(duration / 16, 0.4); /* 16 "frames" distribuídos */
      if (video.videoWidth && video.videoHeight) {
        thumb.style.aspectRatio = `${video.videoWidth} / ${video.videoHeight}`;
      }
      /* Seek para frame inicial (5% do vídeo) */
      frameTime = duration * 0.05;
      if (video.fastSeek) video.fastSeek(frameTime);
      else video.currentTime = frameTime;
    });

    /* Fallback se o vídeo já estava carregado antes do listener */
    if (video.readyState >= 1 && video.videoWidth) {
      duration = video.duration || 0;
      step = Math.max(duration / 16, 0.4);
      thumb.style.aspectRatio = `${video.videoWidth} / ${video.videoHeight}`;
      if (duration > 0) {
        frameTime = duration * 0.05;
        if (video.fastSeek) video.fastSeek(frameTime);
